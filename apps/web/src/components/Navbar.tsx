...
import { CURRENCIES, CurrencyCode } from '@rentwise/shared';
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    // These anchors only exist on the calculator page, so from any other route
    // go home first and scroll once the section has rendered.
    if (!isHome) {
      navigate('/');
            ...
            {isAuthenticated ? (
              <>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-colors"
                >
              ...
              <>
                <Link
                  to="/login"
                  className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-slate-700 hover:text-brand-700 hover:bg-slate-100 rounded-xl transition-colors"
                >
            ...
            {isAuthenticated ? (
              <div className="space-y-2">
                <Link
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-semibold text-white bg-slate-900 hover:bg-brand-600 transition-colors shadow-md"
                >
                ...
                <button
                  type="button"
                  onClick={handleLogout}
                  className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl text-sm font-semibold text-slate-600 bg-slate-100 hover:bg-rose-50 hover:text-rose-600 transition-colors"
                >
